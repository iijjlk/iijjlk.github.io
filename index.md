# About Me

I am a first-year Ph.D. student in Computer Science and Technology at Fuzhou University.

My research interests include low-level image enhancement, event-based vision, and multimodal perception. I am currently exploring how event signals, image priors, and multimodal representations can be used to enhance degraded visual data under challenging conditions.

# Research Interests

- Low-level image enhancement
- Event-based vision
- Multimodal perception
- Low-light image and video restoration

# News

- [2026] One paper was accepted by CVPR 2026.
- [2026] One paper was accepted by Pattern Recognition.
- [2026] One paper was accepted by IEEE TCSVT.
- [2025] One paper was accepted by AAAI 2025.
- [2024] One paper was published in IEEE TGRS.
- [2024] One paper was published in IEEE TIM.

# Publications

1. **BiEvLight: Bi-level Learning of Task-Aware Event Refinement for Low-Light Image Enhancement**  
   **Zishu Yao**, Xiang-Xiang Su, Shengning Zhou, Guang-Yong Chen, Guodong Fan, Xing Chen  
   *Conference on Computer Vision and Pattern Recognition (CVPR), 2026.*  
   [[Paper](https://arxiv.org/abs/2603.04975)] [[Code](https://github.com/iijjlk/BiEvlight)]

2. **Spatio-temporal Collaborative Optimization for Event-guided Low-light Video Enhancement**  
   **Zishu Yao**, Xiang-Xiang Su, Shengning Zhou, Guangyu Zhu, Guodong Fan, Jing Chen  
   *Pattern Recognition, 2026.*

3. **Dual Guidance of Visual and Semantic Information for Real-World Scene Text Image Super-resolution: A Novel Approach and Benchmark Dataset**  
   Rui-Lin Shi, **Zishu Yao**, Fei-Yan Chen, et al.  
   *IEEE Transactions on Circuits and Systems for Video Technology (TCSVT), 2026.*

4. **Divide-and-Conquer: Frequency-Domain Decoupling Strategy for Low-Light Remote Sensing Image Enhancement**  
   Bosong Zhuang, **Zishu Yao**, Guodong Fan, Jinjiang Li  
   *IEEE Journal of Selected Topics in Applied Earth Observations and Remote Sensing (JSTARS), 2025.*

5. **IniRetinex: Rethinking Retinex-type Low-Light Image Enhancer via Initialization Perspective**  
   Guodong Fan, **Zishu Yao**, Guang-Yong Chen, Jian-Nan Su, Min Gan  
   *AAAI Conference on Artificial Intelligence (AAAI), 2025.*  
   [[Paper](https://ojs.aaai.org/index.php/AAAI/article/view/32289)]

6. **Illumination-aware and Structure-guided Transformer for Low-Light Image Enhancement**  
   Guodong Fan, **Zishu Yao**, Min Gan  
   *Computer Vision and Image Understanding (CVIU), 2025.*  
   [[Paper](https://www.sciencedirect.com/science/article/pii/S1077314224003576)]

7. **Spatial-Frequency Dual-Domain Feature Fusion Network for Low-Light Remote Sensing Image Enhancement**  
   **Zishu Yao**, Guodong Fan, Jinfu Fan, Min Gan, C. L. Philip Chen  
   *IEEE Transactions on Geoscience and Remote Sensing (TGRS), 2024.*  
   [[Paper](https://arxiv.org/abs/2404.17400)] [[Code](https://github.com/iijjlk/DFFN)]

8. **GACA: A Gradient-Aware and Contrastive-Adaptive Learning Framework for Low-Light Image Enhancement**  
   **Zishu Yao**, Jian-Nan Su, Guodong Fan, Min Gan, C. L. Philip Chen  
   *IEEE Transactions on Instrumentation and Measurement (TIM), 2024.*  
   [[Code](https://github.com/iijjlk/GACA)]

# Education

- **Fuzhou University**, Fuzhou, China  
  Ph.D. student in Computer Science and Technology  
  2025 - Present

# Contact

- Email: zishuyao98@gmail.com
- ORCID: [0009-0009-9984-0029](https://orcid.org/0009-0009-9984-0029)
- GitHub: [iijjlk](https://github.com/iijjlk)
